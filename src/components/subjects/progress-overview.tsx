...
// Progress variants for category-specific styling
...
// CategoryProgress component for individual category progress bars
const CategoryProgress = memo(
  function CategoryProgress({ title, data, variant, previousValue }: CategoryProgressProps) {
    const theme = progressVariants[variant];
    const progressColor = cn(
      "h-2 rounded-full shadow-sm", // Increased height and added shadow
    ...
    const progressDiff =
      previousValue !== undefined ? data.value - previousValue : 0;
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="text-xs">
                    <p>
                      {progressDiff > 0 ? "Up" : "Down"} by{" "}
                      {Math.abs(progressDiff)}% since last update
                    </p>
...
// Main ProgressOverviewComponent
const ProgressOverviewComponent = ({
  ...
  const progressData = useMemo(() => {
    ...
    return {
      ...
      previousOverall: previousProgress?.overall
        ? validateProgress(previousProgress.overall)
        : undefined,
      categories: {
        learning: {
          value: validateProgress(progress.learning),
          previousValue: previousProgress?.learning
            ? validateProgress(previousProgress.learning)
            : undefined,
        ...
        revision: {
          ...
          previousValue: previousProgress?.revision
            ? validateProgress(previousProgress.revision)
            : undefined,
        ...
        practice: {
          ...
          previousValue: previousProgress?.practice
            ? validateProgress(previousProgress.practice)
            : undefined,
        ...
        test: {
          ...
          previousValue: previousProgress?.test
            ? validateProgress(previousProgress.test)
            : undefined,
  ...
  const overallProgressColor = "h-2 rounded-full bg-white shadow-sm"; // White with shadow for visibility

  const overallDiff =
    progressData.previousOverall !== undefined
      ...
      <div className="space-y-4">
        <div className="space-y-2">
              ...
              {overallDiff !== 0 && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent side="top" className="text-xs">
                      <p>
                        {overallDiff > 0 ? "Up" : "Down"} by{" "}
                        {Math.abs(overallDiff)}% since last update
                      </p>
                      <p className="text-muted-foreground">
                        Previous: {progressData.previousOverall}%
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
          ...
          <div className="relative w-full h-2 bg-muted rounded-full overflow-hidden">
            ...
            <div
              role="progressbar"
              aria-valuenow={progressData.overall}
              aria-valuemin={0}
              aria-valuemax={100}
...
ProgressOverviewComponent.displayName = "ProgressOverviewComponent";
